...
(function () {
     ...
     * Memos
     */
    const allFleetLinks = Array.from(document.querySelectorAll('#fleetList .entry .name a'));
    allFleetLinks.forEach((a) => {
        const m = /\/fleet\/([\d]+)\//.exec(a.getAttribute('href'));
        m && m[1] && a.setAttribute('title', Memo.fleet(m[1]).content);
    });
    const allPlanetsLinks = Array.from(document.querySelectorAll('#fleetList .entry .activity a'));
